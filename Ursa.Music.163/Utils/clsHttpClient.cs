...
public class clsHttpClient
{
    ...
    /// <returns></returns>
    public async Task<string> GetWeb(string url, string date = "")
    {
        string strResult = "";
    ...
    //内部Get 和 Post 方法
    private async Task<string> pGetWeb(string url)
    {
        string strResult = "";

        m_htpResponse = await m_htpClient.GetAsync(new Uri(url));
        strResult = await m_htpResponse.Content.ReadAsStringAsync();

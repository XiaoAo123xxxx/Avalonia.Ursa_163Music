...
{
    private static string m_strGetKgMusicDownloadURL =
        "https://wwwapi.kugou.com/yy/index.php?r=play/getdata&hash=<<FileHash>>&dfid=<<kgDFID>>&mid=<<KgMid>>";
    ...
    //酷狗音乐
    //private static string m_strKgMid = "4c681a7352fe592273400a79644bc30c"; //静态，第一次获取 //mid可为任意值
    private string m_strGetKgMusicList =
        "https://songsearch.kugou.com/song_search_v2?&keyword=<<SongName>>&platform=WebFilter";

    private string m_strGetKwMusicDownloadURL =
        "http://antiserver.kuwo.cn/anti.s?format=mp3|mp3&rid=<<MUSICID>>&type=convert_url&response=res"; //旧下载接口，可下vip
    ...
    private string m_strGetKwMusicList2 =
        "http://www.kuwo.cn/api/www/search/searchMusicBykeyWord?key=<<SongName>>&pn=1&rn=30&reqId=";

    private string m_strGetQQMusicDownloadURL = "http://ws.stream.qqmusic.qq.com/";

    //QQ音乐
    private string m_strGetQQMusicList =
        "https://c.y.qq.com/soso/fcgi-bin/client_search_cp?new_json=1&w=<<SongName>>&format=json";

    private string m_strGetQQMusicVkey =
        "https://u.y.qq.com/cgi-bin/musicu.fcg?&data={\"req_0\":{\"module\":\"vkey.GetVkeyServer\",\"method\":\"CgiGetVkey\",\"param\":{\"guid\":\"958563768\",\"songmid\":[\"<<mid>>\"],\"uin\":\"291116260\"}}}";
    ...
    private string m_strWyyMusicGetDownloadURL =
        "{\"ids\":\"[<<ID>>]\",\"level\":\"standard\",\"encodeType\":\"aac\",\"csrf_token\":\"\"}";

    private string m_strWyyMusicGetList =
        "{\"hlpretag\":\"<span class=\\\"s-fc7\\\">\",\"hlposttag\":\"</span>\",\"s\":\"<<SongName>>\",\"type\":\"1\",\"offset\":\"0\",\"total\":\"true\",\"limit\":\"10\",\"csrf_token\":\"\"}";

    private string m_strWyyMusicIV = "0102030405060708";

    private string m_strWyyMusicRSA =
        ""; //对应 m_strWyyMusicFourth 进行RSA加密后

    private string m_strWyyMusicSecond = "010001";

    private string m_strWyyMusicThrid =
    ...
    /// <returns></returns>
    public async Task<List<clsMusic>> GetMusicList(string SongName)
    {
        ...
        // strJson = await m_htpClient.GetWeb(url);
        ...
        try
        {
            ...
            for (i = 0; i < jo["result"]["songs"].Count(); i++)
            {
                ...
                msc = new clsMusic(i, jo["result"]["songs"][i]["name"].ToString(), "",
                    jo["result"]["songs"][i]["ar"][0]["name"].ToString(),
                    jo["result"]["songs"][i]["al"]["name"].ToString(), enmMusicSource.Wyy,
                    jo["result"]["songs"][i]["id"].ToString(), bolCanDownload);
        ...
        try
        {
            ...
            for (i = 0; i < jo["data"]["song"]["list"].Count(); i++)
            {
                ...
                msc = new clsMusic(i, jo["data"]["song"]["list"][i]["name"].ToString(),
                    jo["data"]["song"]["list"][i]["lyric"].ToString(),
                    jo["data"]["song"]["list"][i]["singer"][0]["name"].ToString(),
                    jo["data"]["song"]["list"][i]["album"]["name"].ToString(), enmMusicSource.QQ,
                    jo["data"]["song"]["list"][i]["mid"].ToString(), bolCanDownload);
    ...
    /// <returns></returns>
    public async Task<string> GetMusicDownloadURL(string date, enmMusicSource emsSource = enmMusicSource.QQ)
    {
        string ResultURL = "";
        string url = "";
        JObject jo;
        ...
        switch (emsSource)
        {
            case enmMusicSource.QQ:
                ...
                ResultURL = await m_htpClient.GetWeb(url);
            ...
            case enmMusicSource.Kg:
                url = m_strGetKgMusicDownloadURL.Replace("<<FileHash>>",
                    date); //.Replace("<<KgMid>>", "c596eb268a2705383a10d0af021664c0");//.Replace("<<KgDFID>>","07u9ob41Vu350chwOw4ejU7b");
                ResultURL = await m_htpClient.GetWeb(url);
